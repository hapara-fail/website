document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateWizardState = () => {
        ...
        if (!currentWizard) {
            ...
            const path = wizardPaths[currentWizard];
            progressContainer.innerHTML = path.map((_, index) => `<div class="progress-step" data-step-index="${index}"></div>`).join('');
    ...
    document.getElementById('select-sipe').addEventListener('click', () => { if(isAnimating) return; currentWizard = 'sipe'; currentStepIndex = 0; updateWizardState(); });
    document.getElementById('select-nppe').addEventListener('click', () => { if(isAnimating) return; currentWizard = 'nppe'; currentStepIndex = 0; updateWizardState(); });
    ...
    function hexEncode(text) { return text.split("").map((c) => c.charCodeAt(0).toString(16).padStart(2, "0")).join("").toUpperCase(); }
    function hexDecode(hex) {
        if (!hex || typeof hex !== 'string' || !(/^[0-9A-Fa-f]*$/i.test(hex)) || hex.length % 2 !== 0) return hex;
        try { return hex.match(/.{1,2}/g).map(byte => String.fromCharCode(parseInt(byte, 16))).join(""); } catch (e) { return hex; }
    }
    ...
    function extractSipe() {
        ...
        if (!jsonData) return showNotification("Input Missing", "Please paste the JSON data first.", "error");
    ...
    function analyzeNppeLog(file) {
        nppeOutputTable.innerHTML = '<thead><tr><th>SSID</th><th>Details</th><th>Security</th></tr></thead><tbody></tbody>';
        ...
        reader.onload = (e) => {
            try {
                ...
                if (networksFound === 0) throw new Error("No network configurations found in log. Ensure 'Include raw bytes' was on and policies were reloaded.");
                showNotification("Analysis Complete", `Found ${networksFound} network configuration(s).`, "success");
            } catch (err) {
                showNotification("Analysis Error", err.message || "Failed to analyze log file.", "error");
            }
        };
        reader.onerror = () => showNotification("File Error", "Could not read the uploaded file.", "error");
    }
    nav.style.display = 'none';
});