document.addEventListener('DOMContentLoaded', () => {
  ...
  function parseSource(source) {
    const initialDataMatch = source.match(
      /(?:var\s+)?_docs_flag_initialData\s*=\s*(\{.*?\});?<\/script>/i
    );
    if (!initialDataMatch || !initialDataMatch[1]) {
      throw new Error(
        "Could not find '_docs_flag_initialData'. The form source might be incomplete or incorrect."
      );
    }
    ...
    if (!info_map?.info_params?.token || !info_map['docs-crp']) {
      ...
      throw new Error(
        'Essential data (token or form path) is missing. Ensure you copied the entire page source.'
      );
    }
    ...
    const url =
      baseUrl + formPath + (my_query_params.toString() ? '?' + my_query_params.toString() : '');
