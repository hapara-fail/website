...
- `Content-Security-Policy` — restricts asset origins; allows connections to `monitor.dns2.hapara.fail` for DNS status checks, and `cdn.jsdelivr.net` with `raw.githubusercontent.com` fallback for blocklist content.
