...
// Track the scroll handler so we can remove it before each page swap
...
function initNav() {
  ...
  if (header) {
    ...
    // Remove any previously registered scroll handler before adding a new one
    if (_navScrollHandler) {
      window.removeEventListener('scroll', _navScrollHandler, { passive: true });
    }
    _navScrollHandler = requestTick;
...
// Re-run nav init after every client-side navigation (and on first load)
document.addEventListener('astro:page-load', initNav);

// Clean up the scroll listener and body-lock before the DOM is swapped out
document.addEventListener('astro:before-swap', () => {
  if (_navScrollHandler) {
    window.removeEventListener('scroll', _navScrollHandler, { passive: true });
    _navScrollHandler = null;
  }
  // Release body-lock if user navigated while the nav drawer was open
  document.body.classList.remove('body-lock');
});
